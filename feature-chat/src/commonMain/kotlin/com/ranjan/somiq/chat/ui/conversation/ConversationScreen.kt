...
import com.ranjan.somiq.chat.ui.conversation.ConversationContract.Action
...
fun ConversationScreen(
    ...
    modifier: Modifier = Modifier
        ...
        // Call actions row (when we have other user)
        if (uiState.otherUserId.isNotBlank() && !uiState.isLoading) {
            Row(
                modifier = Modifier.fillMaxWidth().padding(8.dp),
                horizontalArrangement = Arrangement.End
            ) {
                IconButton(onClick = { onAction(Action.StartVoiceCall) }) {
                    Icon(Icons.Default.Call, contentDescription = "Voice call")
                }
                IconButton(onClick = { onAction(Action.StartVideoCall) }) {
        ...
        if (uiState.isLoading && uiState.messages.isEmpty()) {
            ...
            Row(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onAction(Action.MessageTextChange(it)) },
                ...
                androidx.compose.material3.Button(
                    onClick = { onAction(Action.SendMessage) },
                    enabled = uiState.messageText.isNotBlank() && !uiState.sending
                ) {
                    if (uiState.sending) CircularProgressIndicator(Modifier.padding(4.dp))
