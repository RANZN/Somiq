...
class StoryRepositoryImpl(
    ...
    override suspend fun getStories(): Result<List<Story>> {
        return safeApiCall(
            apiCall = { httpClient.get("v1/stories") },
            onSuccess = { response ->
    ...
    override suspend fun getMyStories(): Result<List<Story>> {
        return safeApiCall(
            apiCall = { httpClient.get("v1/stories/me") },
            onSuccess = { response ->
    ...
    override suspend fun getUserStories(userId: String): Result<List<Story>> {
        return safeApiCall(
            apiCall = { httpClient.get("v1/stories/user/$userId") },
            onSuccess = { response ->
                response.body<List<StoryDto>>().map { it.toDomain() }
            }
        )
    ...
    override suspend fun getStory(storyId: String): Result<Story> {
        return safeApiCall(
            apiCall = { httpClient.get("v1/stories/$storyId") },
            onSuccess = { response ->
